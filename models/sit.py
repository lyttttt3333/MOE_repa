...
from models.moe_models import PTMHunYuanMoE
...
class SiTBlock(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, latent_shape = (1,16,16), **block_kwargs):
        super().__init__()
        ...
        moe_config = {**hardcode_config, **reference_config}

        self.latent_shape = latent_shape
        self.core_shape = moe_config["core_shape"]

        self.moe_layer = PTMHunYuanMoE(**moe_config)
        # self.test_model = nn.Linear(mlp_hidden_dim, hidden_size)
    ...
    def forward(self, x, c):
        ...
        ffn_output, l_loss, z_loss = self.partition_3d_moe(ffn_input)
        # ffn_output = self.mlp(ffn_input)
        x = x + gate_mlp.unsqueeze(1) * ffn_output

        return x, l_loss, z_loss

    def partition_3d_moe(self, x):
        B, N, C = x.shape
        T, H, W = self.latent_shape
        assert T*H*W == N

        x = x.view(B, T, H, W, C)  # (B, T, H, W, C)
        x, l_loss, z_loss = self.moe_layer(x)
        x = x.reshape(B, N, C)
        return x, l_loss, z_loss
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        # input_size, patch_size, in_channels, hidden_size 32 2 4 768
        ...
        self.blocks = nn.ModuleList([
            SiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, latent_shape=(1, input_size//patch_size, input_size//patch_size), **block_kwargs) for _ in range(depth)
        ])
    ...
    def forward(self, x, t, y, return_logvar=False):
        ...
        for i, block in enumerate(self.blocks):
            x, l_loss, z_loss = block(x, c)                      # (N, T, D)
        ...
        l_loss_sum = torch.stack(l_loss_list).mean()
        z_loss_sum = torch.stack(z_loss_list).mean()
        return x, zs, l_loss_sum, z_loss_sum
